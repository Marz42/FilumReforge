...
| **阶段** | **主开发线 F-05 + RC2/UAT 等待** — 部门负责人模板可见性热修已回流；详情数据协调拆分完成，I4/设计器/S-01 等待人工 UAT，I3-F 生产切流仍 gated |
| **最后整理** | 2026-08-11 — `v0.93.0-rc.2` 固定并回流，F-05 主线继续，KI-011 继续延后 |
...
| **F-05** | `TaskDetailShell` 完整拆分 | 🟡 首批数据协调已抽取，约 1,989 → 1,895 行；动作提交与资料/评论板块待后续 |
...
3. **F-05 后续拆分**：在保持界面与业务语义不变的前提下，依次提取动作提交协调和资料/评论板块；每批独立回归与提交。
...
**下一 actionable**：开发侧继续 F-05 动作提交协调拆分；目标环境侧并行执行“数据检查”→“验收准备”→人工 UAT checklist。只有清单必测项完成并签字，才把 I4/设计器/S-01 改为 UAT 通过；RC 与生产准入仍按独立清单推进。
