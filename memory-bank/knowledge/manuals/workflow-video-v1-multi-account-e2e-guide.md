...
paradigma:
  relations:
    related_to:
      ...
      - workflow-video-v1-docker-runbook.md
...
| 负责人下拉显示 UUID / 为空 | 须 `seed_version≥3` 且重跑 `seed_workflow_video_templates`；生产机用 `--copy-dept-code` / `--post-dept-code`（见 [Ubuntu 部署手册 §21.3.1](../manuals/deployment-runbook-ubuntu-2404.md)） |
