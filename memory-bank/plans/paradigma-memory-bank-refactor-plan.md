...
| 项 | 决定 |
|----|------|
| `handbooks/` 命名 | **保留**；仅在 README 标注与 Paradigma `manuals/` 等价 |
...
当前 FilumReforge 的 `memory-bank/` 在 2026-05-14 已完成一次目录重组（`handbooks/`、`plans/`、`history/`、`archive/`），工程文档质量高、与实现对齐紧密，但存在以下 **Agent 协作层面** 的痛点：
...
> **命名说明**: `handbooks/` → `manuals/` 是为与 Paradigma 术语对齐；可通过 README 保留旧路径别名说明一个版本周期，减少外链断裂。
...
| `handbooks/*` | `manuals/*` | **重命名目录** + README 留 redirect |
...
- [x] 评审本方案，确认保留 `handbooks/`（仅 README 标注与 `manuals/` 等价）
...
| 外链断裂（handbooks 重命名） | CI/文档链接 404 | Phase 4 全仓库搜索；可暂留 `handbooks/README.md` stub 指向 manuals |
...
4. 全仓库无断裂的 `handbooks/` 引用（或均有 redirect）
