...
>
> **协议基座**: 基于 [Marz42/paradigma](https://github.com/Marz42/paradigma) 定制；**Phase 0–4** 已完成。
...
- `memory-bank/architecture.md` — 工程蓝图：模块、运行时、核心流程
- `memory-bank/data-contracts.md` — schema、枚举、实体关系、API 索引
- `memory-bank/conventions.md` — 编码与协作规范
- `memory-bank/active-task.md` — 当前唯一聚焦任务
- `memory-bank/progress.md` — 会话摘要 + 阶段验收与测试基线

## 🌡️ WARM（按需加载）

- `memory-bank/roadmap.md` — 宏观里程碑与当前版本焦点
- `memory-bank/changelog.md` — 版本发布历史
- `memory-bank/domains/*.md` — 子系统领域文档（任务涉及相关域时读取）
- `memory-bank/plans/*.md` — 细粒度实施计划
- `memory-bank/design-document.md`、`tech-stack.md` — 完整叙述（摘要在 project-brief）
- `backend/README.md`、`frontend/README.md`、`infra/docker/README.md`
- `memory-bank/handbooks/user-manual.md` — 用户说明书

## 🧊 COLD（仅排查时读取）

- `memory-bank/decisions.md` — 架构决策 (ADR)
- `memory-bank/known-issues.md` — 已知坑位与环境陷阱
- `memory-bank/glossary.md` — 项目术语表
- `memory-bank/handbooks/*.md` — 运维手册（≈ Paradigma `manuals/`）
...
- 🔥 所有 HOT 文件（见上表；迁移期含现行路径）
- 🌡️ 与当前任务相关的 WARM 文件（如 workflow-video 任务 → 对应 `plans/workflow-video-v1-*.md`）
...
- 写代码前，用简短中文描述修改思路。
- 若修改涉及数据库 schema 或跨模块 API（破坏 `data-contracts.md`），**必须先征得用户同意**。
- 若涉及架构决策，完成后应记录（Phase 2 起写入 `decisions.md`；当前可写入相关 `plans/*-adr.md`）。
...
1. **追加 `memory-bank/progress.md`**（文首「会话摘要」区），记录：完成事项、踩坑、遗留、下一步；时间戳通过工具获取

### 当涉及实质性修改时

2. schema / 枚举变化 → 更新 `memory-bank/data-contracts.md`
3. 模块、运行时、流程变化 → 更新 `memory-bank/architecture.md`
4. 阶段状态、验测结论 → 更新 `memory-bank/progress.md`
5. 产品边界 → 更新 `memory-bank/project-brief.md`
6. 更新 `memory-bank/active-task.md` check-list
7. **版本号评估**（SemVer，见 `VERSION` 与 `conventions.md`）：
   - 纯文档/注释 → 通常不升版本
   - Bug 修复（API 不变）→ PATCH
   - 向下兼容新功能 → MINOR（Agent 建议，用户确认）
   - 不兼容 API / schema → MAJOR（**须用户明确同意**）
   - 需要时：更新 `VERSION` + `memory-bank/changelog.md` + 在 progress 记录
...
| 生产部署 | `memory-bank/handbooks/deployment-runbook-ubuntu-2404.md`、`scripts/check-release.sh` |
