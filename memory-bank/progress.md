...
| Phase 11-G / 文档收口、前端回归与 Playwright 基线 | in_progress | 已完成 docs gate：11-G 范围从“纯文档收口与全量回归”扩写为“文档收口 + 前端测试补强 + Playwright 浏览器端到端基线 + 最终全量回归”。本轮明确只覆盖现有页面与真实点击流，不做视觉回归，不新增完整 workflow graph 前端可视化 UI，也暂不把 Playwright 并入默认发布闸门；下一步进入前端可测性加固、Playwright 框架接入与基础 E2E 用例实现。 |
