...
| IA-1 / UI Phase A（登录与设置） | done | **已完成待审阅**：三场景登录（A/B/C）、密码错误提示统一、设置三分栏（`/settings/profile|security|notifications`）、`POST /auth/change-password`；测试：`passwordPolicy.test.ts`、`formErrors.test.ts`、`LoginView.spec.ts`、`e2e/login.spec.ts`、`e2e/settings.spec.ts`。 |
| IA-1～IA-4 / UI 重构规格 v2 | in_progress | Phase A 完成；下一步 Phase B（壳层铃铛）见 [`ui-refactor-spec-v2.md`](plans/ui-refactor-spec-v2.md)。 |
