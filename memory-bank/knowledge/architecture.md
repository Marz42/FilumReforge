...
- 工作流图引擎 schema 现为 **十一表**：原九表 + `workflow_edge_traversals` + `workflow_node_activation_dependencies`。新模板 Run 使用 snapshot format v2 / `graph-v3`，按实际经过边与产生的 activation 推进；既有 `graph-v2` 与 `legacy-v1` 不原地升级。Phase 3 起的 Task dual-write 与 graph-first fallback 仍保留，写所有权收口属于后续 Iteration 3
