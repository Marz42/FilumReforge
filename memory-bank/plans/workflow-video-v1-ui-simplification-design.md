...
| AR-02 | Legacy E 与图模板 Tab 并存 | 已知双轨架构（ADR-005） | 实测路径不清晰 |
...
- [ ] 跟踪页对已提交选题可 **指派并启动制作**，无需等 3/3  
- [ ] 同一选题不可重复 fork（UI 禁用 + API 409）  
- [ ] `submit_mode=file` 节点主按钮为上传提交  
- [ ] **更多 → 退回** 可用且用户态变为 **已退回**  
- [ ] 实例化默认 **指定成员**，派发后进入跟踪 Tab  

### 11.3 P2

- [ ] 列表 / 看板 / 甘特独立组件，按 §7.2 规格  
- [ ] **任务统计** 入口可查看 Run 事件与部门汇总  
- [ ] 详情内引擎态 / 全量日志已迁出或默认折叠  
