...
title: "Project Filum 进度记录"
description: "会话摘要合集 + 阶段验收与测试基线。"
tags:
  - progress
  - session-log
...
**完成事项**:
- [x] 将根目录升级意图与现行九表图引擎、Task 双写、模板管理、条件 Join、Outbox、权限与测试基线逐项对照
- [x] 新增 [`knowledge/plans/workflow-graph-engine-upgrade-iteration-plan.md`](../../knowledge/plans/workflow-graph-engine-upgrade-iteration-plan.md)
- [x] 将顺序收敛为：基线冻结 → 安全/版本冻结 → 路径与完成语义 → 写所有权/幂等 → Handler → 投影/运维 → Legacy 清理
- [x] 明确 Task 暂不物理改名、既有审批优先复用、先路径账本后完整 Token、在途 Run 固化 engine version 的务实策略
- [x] 补充对象级授权、部门 scope 绕过、PostgreSQL 并发验证、Expand/Backfill/Shadow/Cutover/Contract 与禁止切流条件
- [x] 同步 `runtime/active-task.md` 与实施计划当前执行位置

**状态**：仅方案与 memory-bank 更新，未修改 schema、API 或业务代码；待用户确认计划 §12 后，才可启动 Iteration 0。

**遗留/下一步**：用户确认十项推荐决策；S-01 统计 Tab 仍待验收。
...
**现象**：部署 `0.91.1` 后 `GET /task-center` 报 `AttributeError: WorkflowGraphInstanceStatus has no attribute 'SUSPENDED'`。

**修复**：批次 ROOT 进行中态改为 `ACTIVE`/`PENDING` → `DOING`（枚举实际值见 `enums.py`）。
...
**修复**：`_is_batch_graph_root_shell_task`；批次 ROOT 以 `WorkflowGraphInstance.status` 为准（`ACTIVE`/`PENDING` → `DOING`；`COMPLETED` → `DONE`）；阶段文案「汇总派发：待确认派发」。
...
**F-29**：`POST /api/v1/tasks/{id}/archive`；软归档 + 图 Run CANCELLED；详情「归档任务…」；归档后跳过详情 reload（避免 MissingGreenlet）。

**Admin 跟踪**：`list_task_tracking` 对 Admin/HR 展示全量未完成任务（「督办」标签）；修复 `_build_visible_task_statement` watchers 预加载。

**逾期**：跟踪/详情「延期…」；已逾期须设更晚 `due_date`；明确逾期不阻断推进。

**测试**：`test_f29_admin_task_archive.py` · `test_task_center_management_tracking.py`。
...
### 2026-06-23 — F-29 归档 API + 详情入口

**后端**：`POST /api/v1/tasks/{task_id}/archive`（admin only）；软归档 `extra_metadata.admin_archived*`；关联 `WorkflowGraphInstance` → `CANCELLED`，未完成节点 → `TERMINATED`；批次 ROOT 额外取消 ACTIVE/PENDING 子 Run。inbox/tracking 过滤已归档任务；已归档任务拒绝交付操作。

**前端**：任务详情「更多 → 归档任务…」对话框（admin、未归档任务）；非视频流任务 admin 也可见「更多」。

**测试**：`backend/tests/test_f29_admin_task_archive.py` 3 用例通过。
...
- [x] [`decisions.md`](./decisions/decisions.md) **ADR-010** 任务流决策
- [x] [`active-task.md`](../runtime/active-task.md) 下一焦点同步
...
- [x] 采纳 G-01–G-06 决策并写入 [`roadmap.md`](./roadmap.md)、[`decisions.md`](./decisions/decisions.md) ADR-009
- [x] 更新 [`domains/task-center.md`](./domains/task-center.md) §6.0–6.1、§10；[`active-task.md`](../runtime/active-task.md)、[`architecture.md`](./architecture.md)、[`project-brief.md`](./project-brief.md)、[`known-issues.md`](./known-issues/known-issues.md)
...
- [x] [`data-contracts.md`](./contracts/data-contracts.md)：设计器 API 索引 + 测试基线刷新
- [x] [`architecture.md`](./architecture.md)：`GraphTemplateDesignerView` / AdminService；TCE backlog 状态修正
- [x] [`decisions.md`](./decisions/decisions.md) ADR-008 图模板设计器 authoring
- [x] [`known-issues.md`](./known-issues/known-issues.md)、[`README.md`](./README.md)、[`plans/task-center-enhance.md`](./plans/task-center-enhance.md)、[`domains/workflow-graph-engine.md`](./domains/workflow-graph-engine.md)
...
**完成事项**:
- [x] [`active-task.md`](../runtime/active-task.md) 切换为 **TCE Phase 1**（B-01/B-03/B-02 + F-02/F-03/F-08；PR-A/PR-B）
...
- [x] [`architecture.md`](./architecture.md) §6.13.14 TCE backlog；[`known-issues.md`](./known-issues/known-issues.md) 读模型/实例化部门
...
- [x] plans/knowledge/manuals/archive 内 schema 维护指向 `data-contracts.md`
...
> 命令索引：`frontend/package.json` · 手册 [`knowledge/manuals/workflow-video-v1-multi-account-e2e-guide.md`](knowledge/manuals/workflow-video-v1-multi-account-e2e-guide.md)
...
**多账号 E2E 收口（2026-05-19）**：指南 [`knowledge/manuals/workflow-video-v1-multi-account-e2e-guide.md`](knowledge/manuals/workflow-video-v1-multi-account-e2e-guide.md)。实现侧：`WorkflowVideoInstantiationService` 实例化/生产 fork `commit`；`TemplateInstantiateDialog` 参与人来自 `preview-participants`；`TemplateAggregatePanel` 合并 script_author 选项；`workflow_rule_resolver` 支持 `department_pool.assignee_role: member`；生产 fork 仅 start 节点解析 assignee。Playwright 导航：finalize 后批次 ROOT 在 **历史** Tab；E/F 按 **选题标题** 打开制作 Run 根任务并点击列表行。
...
| IA-0 / 用户说明书 v1 | done | [`memory-bank/knowledge/manuals/user-manual.md`](knowledge/manuals/user-manual.md) 已成稿并完成审阅批注；作为 UI 重构事实基线。 |
...
按 [`knowledge/manuals/deployment-runbook-ubuntu-2404.md`](knowledge/manuals/deployment-runbook-ubuntu-2404.md) §19–§20，**在线生产/预发主机**已完成首轮上线验证（用户确认，2026-05-21）：
...
- 阶段范围与排期以 `memory-bank/plans/improvements-stage2-implementation-plan.md` 为准；**文档物理路径**以 `memory-bank/README.md` 索引为准（`knowledge/manuals/`、`plans/`、`history/`、`archive/outdated/`）。**对齐审查报告**默认写入 `memory-bank/history/reports/alignment-assessment-YYYYMMDD.md`（见 `.github/prompts/memory-bank-alignment-review.prompt.md`）。
...
Stage 2、UI IA、工作流图引擎 Phase 11、**TCE Phase 1–5**、**图模板设计器 D1–D3 + UX 抛光** @ `0.89.0` 均已收口。任务中心 **产品闭环已完成**；下一工程主线为 **架构债与工程质量**（[`active-task.md`](../runtime/active-task.md)）：
...
- **生产部署**：Ubuntu systemd 见 [deployment-runbook §21.3.1](./manuals/deployment-runbook-ubuntu-2404.md)；示例 `--copy-dept-code Div.Alpha --post-dept-code Div.Echo`
...
## 2026-06-23 · N7 指派剪辑师列表错部门

- **根因**：旧制作 Run 的 `schema_snapshot` 无 `pool_key` → 前端回退 `managed-department-member-options`；部门池 API 未读实例 `context.department_pools`
- **修复**：`resolveUserPoolKey` 默认 N7→`post_production`；池成员 API 支持 `instance_id`（commit `314bb6f`）

## 2026-06-23 · 文档 · F-29 管理员任务治理立项

- **roadmap / active-task / domains §15**：整理管理员现状；下一项 **F-29** 单条任务 archive API + UI
- **commit** `efa450c` N10 尾节点修复（待 push）

## 2026-06-23 · N10 完成后误归档（跳过 N11–N12）

- **根因**：旧制作 Run 缺 N11/N12 节点实例，或 N10 `template_node_id` 与模板边脱节 → `_activate_downstream` 找不到出边 → `_maybe_complete_instance` 误判全完成并 `archived=true`
- **修复**：激活下游前 materialize 缺失模板节点；按 `node_key` 回退解析出边；完成判定要求模板全部 `node_key` 均有 COMPLETED 实例
- **测试**：`test_w4p_n10_*` 覆盖 N10→N11、缺尾节点补建、stale template_node_id
## 2026-07-13 · 工作流图引擎 Iteration 0 基线实施

- 新增独立 `workflow_gap` 测试套件：`WG-GAP-001`–`004`、`AUTH-GAP-001`–`003`，共 11 个 strict xfail 参数实例；另有 1 个无 xfail 基座自检。
- PostgreSQL 基座扩展：随机临时库、Alembic head、多个独立 AsyncSession、强制断连删除、残留校验；`FILUM_REQUIRE_POSTGRES_TESTS=true` 时所有 postgres skip 转失败。
- 新增 PostgreSQL 并发证据：双上游汇合、同节点重复完成、Deep-Reject/完成竞争，以及临时库无残留。
- 新增统一基线报告，冻结目标权限策略并盘点 Task/Runtime 双写、commit/UoW、Outbox 重复窗口和 Iteration 1–3 影响。
- 新增 proposed ADR-012–016；用户批准前维持“提议”，不进入 Iteration 1。
- 初次本机 PostgreSQL 未运行，普通模式按契约 5 skip；随后启动仓库 Docker PostgreSQL，强制模式 5/5 通过，无 `PG-GAP-*`，测试后已停止容器与 Docker Desktop。
- 边界检查：未修改 `backend/app/`、Alembic revisions、前端、公共 API 或数据库 schema。

## 2026-07-13 · Iteration 0 用户验收

- 用户统一采纳 ADR-012–016；五份 ADR 从“提议”转为“已采纳”。
- Iteration 0 测试、PostgreSQL、基线报告和决策闸门全部完成。
- 下一步进入 Iteration 1：对象级授权、模板 scope、发布定义不可变、Run snapshot/hash/engine version 与 legacy executor。

## 2026-07-13 · Iteration 1 启动 / I1-A 对象级授权

- 新增中心化 `WorkflowAccessPolicy`，对象读无权统一 404。
- Run 详情、事件、子 Run、submission 统一校验发起人、当前/历史办理人、管理子树、正式 watcher、Admin/HR。
- designer、模板统计和模板实例列表统一要求模板管理能力。
- `AUTH-GAP-001`–`003` 的 7 个 strict xfail 参数实例转为正常通过回归；`workflow_gap` 仅剩 4 个运行时语义缺口。
- 增加发起人、当前/历史办理人、部门经理、watcher 与模板管理经理正向测试。
- 验证：API 专项 44 项通过；后端全量通过；compileall 通过。

## 2026-07-13 · Iteration 1 / I1-B–E 定义冻结与 executor 路由

- `ACTIVE/ARCHIVED` 模板禁止 update/delete/import/整包保存；内置视频 seed 更新改为归档旧 ACTIVE 并派生新版本。
- 新增显式 `scope_mode=global|departments`；存量按部门列表回填，Run 创建解析最终部门后校验 scope；模板管理读取限制在经理有效管理子树。
- 迁移 `20260713_01` 增加 scope、definition snapshot/hash、engine/executor 字段；存量 Run 保持 `legacy/legacy-v1`。
- 新 Run 写 canonical snapshot（node-key 边）和 SHA-256，使用 `snapshot/graph-v2`；图推进、深度打回、模板链、投影任务、表单和 fork 配置均按快照读取，并在执行前校验 hash。
- 新增只读 `report_workflow_legacy_runs`，按模板版本统计 active legacy Run、缺失模板与不可证明定义。
- `WG-GAP-004` 转为正常回归；`workflow_gap` 仅剩 Iteration 2 的 `WG-GAP-001`–`003` strict xfail。
- 验证：后端非 PostgreSQL 全量通过；PostgreSQL 强制模式 5/5（Alembic 往返、JSONB snapshot、并发与临时库清理）通过；前端 type-check 与 Vitest 55 文件/146 项通过；compileall 与 memory-bank 全检通过。

## 2026-07-15 19:39 · Iteration 2 路径与完成语义

- 新 Run 升级为 snapshot format v2 / `graph-v3`；既有 graph-v2 与 legacy-v1 executor 保持不变。
- 迁移 `20260715_01` 新增显式 routing mode、Run result/diagnostics、traversal 与 activation dependency 两张路径账本表，并扩展 skipped/failed/suspended。
- 运行时持久化 taken/not-taken 条件证据；未选专属路径 skipped；exclusive/inclusive/parallel/first-match、Wait-All/Wait-Any、no-route 与合法 End 完成策略落地。
- Deep-Reject 失效旧 traversal/dependency，旧 iteration 完成/推进均返回冲突；Context patch 要求 expected version并写 diff event；多人 fan-out 使用 `branch:NNNN` 不可变身份。
- PostgreSQL Wait-Any 测试发现并修复 Node→Run 锁顺序死锁，统一为 Run→Node。
- 验证：`WG-GAP-001`–`003` 全部转正；后端全量通过；PostgreSQL 强制模式 10/10（Alembic head↔base + 并发矩阵）通过且无残留库；前端 `vue-tsc --build`、Vitest 55 文件/146 项、Vite production build 与 compileall 通过。
- 环境：系统 npm 启动器损坏，改用工作区 Node 直接运行 vue-tsc；测试后已删除临时 PostgreSQL 容器并关闭 Docker Desktop。下一步等待用户验收，不进入 Iteration 3。

## 2026-07-17 · P0 Standalone Work Item 回归矩阵固化

- 新增 `test_standalone_work_item_e2e.py`：API 级场景 A–F（14 passed）— 完整生命周期、返工、转办、候选授权、列表去重、`handshake` 拒绝。
- 新增 `test_standalone_work_item_postgres_concurrency.py`：PostgreSQL 真并发 5 项 — assignee/ADMIN 双转办、delegate vs submit/start、REVIEW inbox 并发读。
- 扩展 `test_standalone_work_item.py`：assignment_mode 不变量、action_policy 无副作用、REVIEW 创建人离职 ADMIN 兜底。
- 服务层：`task_admin_override` 命名；ADMIN 转办 `delegated_by_admin` 审计；standalone 状态命令 `FOR UPDATE` + assignee CAS；转办目标 inactive 改 409 而非 401。
- 验证：E2E+unit 23 passed；`-m postgres` strict **22/22**；全量 backend **311 passed**；Alembic head↔base 含 `20260717_01`。
- 验收报告：[`p0-standalone-work-item-acceptance-20260717`](../history/reports/p0-standalone-work-item-acceptance-20260717.md)。P0 批待用户 review 后冻结提交；批次二（Nginx/Markdown）blocked。

## 2026-07-17 · P0 冻结提交 + 批次二 Nginx/Markdown

- P0 已提交：`8d64a7c` `feat(tasks): treat standalone work items as first-class`。
- 开发 Nginx `default.conf` 增加 server 级 `client_max_body_size 64m`；与 compose/prod 配置一致性测试 + 附件大小边界测试（`test_upload_limits.py`）。
- 新增 `frontend/src/utils/safe-markdown.ts`（marked → DOMPurify）；知识库正文与附件 Markdown/docx 预览统一消毒；RAG 上下文保持纯文本。
- XSS 单测覆盖 script / onerror / javascript: / iframe；Vitest 13 passed；vue-tsc 通过；开发 nginx 已 reload。
- 下一步：批次二单独 commit 验收。

## 2026-07-17 · P1-10 模板图任务防自审

- 模板图任务验收激活默认拒绝执行人自审；候选按直属上级 → 部门负责人 → 工作流管理员（模板创建人/Run 发起人）→ 系统管理员解析，并为每个排除项写 `task_logs`。
- 候选耗尽时任务进入 `blocked`，持久化 `blocked_reason=no_eligible_reviewer`；不自动降级为自审。新增 admin-only `PUT /api/v1/tasks/{id}/reassign-reviewer` 作为显式审计改派。
- standalone 任务验收策略保持不变；Legacy E → graph 迁移的 REVIEW 任务同步执行新验收人解析。
- 验证：`python3 -m pytest tests/ -v --tb=short -x` **393 passed / 10 skipped**；`compileall`、前端 `vue-tsc --build`、`pd-check-all.py` 通过。

## 2026-07-18 00:22 · Task Center P0–P2 审计修复收口

- **P0（`cba64aa`）**：为任务更新、状态流转、交付提交与转办补齐 `FOR UPDATE`/CAS；非法模板步骤优先级改为稳定 422 校验错误。专项回归 **41/41 passed**。
- **P1 batch 1（`3002275`）**：列表游标排序补 UUID 最终 tiebreaker；评论附件绑定改为单次原子提交；截止时间比较统一归一化为 aware UTC。专项回归 **28/28 passed**。
- **P1-10（`4398439`）**：模板图任务禁止执行人自审；验收候选按直属上级 → 部门负责人 → 工作流管理员 → 系统管理员兜底，无候选时持久阻塞，并提供管理员审计改派。Backend 全量 **393 passed / 10 skipped**。
- **P2-11/P2-13**：`_graph_task_projection_map` 的 UUID 回退查询增加 `source_type="task"` 约束，避免跨来源 UUID 碰撞；旧自审权限分支补充历史用途及 P1-10 替代机制注释，行为不变。
- **完成事项**：Task Center 本轮 P0/P1/P2 审计问题全部关闭；版本评估为 API 不变的 bugfix，PATCH 升级至 `0.92.1`。
- **验证**：Backend 全量 **393 passed / 10 skipped / 1 warning**；Frontend 全量 **57 files / 158 tests passed**。Frontend 首轮在并行负载下 `ProfilesView.spec.ts` 超过其显式 15 秒限制，单文件仅 2.8 秒；将该重型组件用例的局部超时放宽至 30 秒后，仍以要求的全量命令 `--test-timeout=15000` 通过。Paradigma **All 5 checks passed**。
- **遗留/下一步**：无本轮遗留；Iteration 3-F 目标环境七天 readiness 证据仍按既有计划独立推进。

## 2026-07-20 · 记档 KI-009（C 闭环验收中发现）

- C1.1：L2 创建并指派 L4 后跟踪可见正确，但详情仍可「开始处理」（前后端旧授权/旧状态按钮未消费 `available_actions`）。
- 记入 [`ki-009-standalone-action-dual-track`](../knowledge/known-issues/ki-009-standalone-action-dual-track.md)；同类：提交/验收按钮启发式、`_can_operate_task` 含 creator。
- **处置**：等用户完成 C 业务闭环排查后，再做 standalone 最小收口；不借此全量迁 workflow 契约。

## 2026-07-20 · C 闭环：转办给管理员后待处理不可见

- **现象**：standalone 转办给 `admin` 后，任务只出现在「任务跟踪」，「待处理」看不到；用户感觉没提醒。
- **根因**：
  1. `list_task_tracking` 对 ADMIN/HR **故意清空** `inbox_task_ids`，个人待办也会进跟踪（督办全量），与「inbox ∩ tracking = ∅」冲突，易被当成「只有跟踪有」。
  2. 管理员可见范围是全组织且按 `created_at` 扫描，旧任务被转办过来时可能落在 scan window 外，inbox 漏收。
