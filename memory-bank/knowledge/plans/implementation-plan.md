...
**当前执行位置**: **工作流图引擎 Iteration 4 A–E、F-05、Iteration 5-A～E、KI-015 请求侧缺口遥测与 2026-08 安全修复均已完成本地工程实现**；`v0.93.0-rc.3` 已在 `21975a6` 固定。5-E 已提供投影优先读取、动态回退和严格 canary，隔离 PostgreSQL/Redis 上完成 22 项方言测试、97/28/282 最终全量重建、407/407 full shadow、fallback on 8/8 与 strict 9/9 真实 UAT，以及备份恢复和 `04→03→04` 演练。KI-015 进一步补齐三类列表 surface 的缺口分类、结构化日志、Admin Operations 诊断和隐私回归。KI-014 Phase A/B 已由 `61ea3d3` 固定；Phase C 强制只读聚合观测入口已完成并在空隔离 PostgreSQL 验证，真实数据完整周期观察与独立 contract migration 仍待目标预发。生产关闭 fallback、真实环境日志告警、ROOT shell 收缩和 Iteration 6 仍受真实预发持续观察、I3-F 连续门禁及人工批准约束。目标环境 I4/设计器/S-01/KI-009 人工 UAT与生产 TLS/secret 仍并行推进。KI-011 按用户决定暂不推进。详见 [`2026-08-26-ki014-schema-drift-remediation-plan.md`](./2026-08-26-ki014-schema-drift-remediation-plan.md)、[`2026-08-11-f05-iteration5-6-sequencing-plan.md`](./2026-08-11-f05-iteration5-6-sequencing-plan.md) 与 [`../contracts/projection-contract.md`](../contracts/projection-contract.md)。
...
5. KI-015 strict 投影缺口请求侧遥测已完成；KI-014 Phase C 只读聚合观测入口已完成，下一门禁是把它运行在含代表性数据的目标预发并覆盖完整业务周期，之后才执行独立 contract；再处理 KI-016 logout 在途请求，KI-017 包体优化进入前端性能批次
