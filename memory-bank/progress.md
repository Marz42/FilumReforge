...
- **根因**：旧制作 Run 的 `schema_snapshot` 无 `pool_key` → 前端回退 `managed-department-member-options`（文案经理可见文案部）；部门池 API 未读实例 `context.department_pools`
- **修复**：`resolveUserPoolKey` 默认 N7→`post_production`；池成员 API 支持 `instance_id` + `resolve_department_pools`
- **运维**：若仍见文案部，检查模板 `department_pools.post_production` UUID 是否指向后期部并重跑 seed
