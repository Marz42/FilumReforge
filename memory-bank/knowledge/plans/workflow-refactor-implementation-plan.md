...
type: paradigma-plan
title: 工作流重构计划
description: "工作流重构 Phase 1–11 实施计划。"
tags:
  - plan
  - 工作流重构
...
3. `memory-bank/logs/progress/0000-legacy-progress.md`：当时的阶段状态、验证命令和用户验收结论；新证据以 Checkpoint 为事实源。
...
1. **生产域名配置硬化**：`FRONTEND_APP_URL` 在 production 环境下改为必填，禁止继续使用 localhost 默认值；并已同步补齐 `backend/.env.production.example`、`infra/docker/.env.prod.example`、`infra/docker/docker-compose.prod.yml`、`memory-bank/knowledge/manuals/deployment-runbook-ubuntu-2404.md`。
...
5. 更新 `memory-bank/knowledge/manuals/deployment-runbook-ubuntu-2404.md`：补齐迁移前备份、`--dry-run`、正式执行、抽样核对、回滚命令与停机/只读窗口建议。
