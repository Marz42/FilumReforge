...
- 修复部门负责人进入任务模板页面时仅请求“可管理模板”，导致全局模板和跨部门共享模板虽具备读取/实例化权限却不显示的问题。
- 模板列表同时加载可读 ACTIVE 模板与可管理模板并按 ID 去重；设计、复制、改名、可用部门、归档和删除操作改为逐模板授权，避免修复可见性时扩大管理权限。
- 增加共享模板“可见、可实例化、不可管理”的组件回归，保持后端部门 scope 对象级授权不变。
