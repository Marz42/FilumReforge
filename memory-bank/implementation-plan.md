...
**当前执行位置**: `refactor-plan.md` 的 **Step 5 / 档案管理 & 用户管理合并（已实现，等待用户验测）**
...
- 当前批次：已完成 Step 5，把 `/people` 从 Users / Profiles tab 壳层升级为统一人员工作台，并新增只读聚合接口 `/api/v1/people-management`；在用户确认前不进入 Step 6
- 收敛前端大型页面的状态与副作用
- 梳理服务层边界，减少跨服务耦合
