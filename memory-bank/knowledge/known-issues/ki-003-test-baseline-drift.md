...
| backend current main | **479 collected / 447 passed / 32 skipped / 0 failed** @ 2026-08-12 | Iteration 5-B checkpoint、projector/rebuild、失败隔离、重建 CLI 与 SQLite expand/downgrade 后的完整汇总 |
...
**本地单元基线 ID**: `2026-08-12-iteration5b-projector-rebuild`；浏览器 E2E 最近完整基线为 `2026-08-09-v0.93.0-rc.1-release-candidate`，RC2/当前主线仍待目标环境刷新。

**Iteration 5-A/B 数据库门禁**：Alembic 单 head `20260812_02`；SQLite pre-projection expand/downgrade PASS；`20260730_01:20260812_01` 与 `20260812_01:20260812_02` PostgreSQL 离线 SQL PASS。真实 PostgreSQL head↔base 因本机无 Docker/PostgreSQL 保持 skip，目标环境须用 `FILUM_REQUIRE_POSTGRES_TESTS=true` 补证。
