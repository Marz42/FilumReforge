...
- `TASK_CENTER_PROJECTION_READS_ENABLED`：默认 `true`，任务中心优先读取 Iteration 5 持久投影。
- `TASK_CENTER_PROJECTION_FALLBACK_ENABLED`：默认 `true`，缺失或旧版投影按原 graph-first 路径动态补读；仅在全量 rebuild + full shadow 达标并完成受控 canary 后设为 `false`。
...
前端源码 bind mount 与 `/app/node_modules` 命名卷相互独立。容器启动时会比较 `package-lock.json` 哈希：锁文件变化后自动执行一次 `npm ci` 更新依赖卷，未变化时直接启动 Vite；因此新增前端依赖后通常无需手工执行 `docker compose down -v`。
