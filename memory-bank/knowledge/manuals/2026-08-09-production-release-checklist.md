...
| A-03 | Backend 460 collected、0 failed；Frontend 64 files/181 tests、type-check、build 通过 | [x] |
| A-04 | `compileall`、Compose `config -q`、Alembic 单 head `20260730_01`、Paradigma 5/5、`git diff --check` 通过 | [x] |
| A-05 | 记录剩余非阻断项：32 个目标环境 skip、809 KB Element Plus 主包；RC2 改动文件 lint 为 0，RC1 快照的测试/E2E 代码仍有 24 条 Oxlint 基线项（当前主线已清理，不反向混入最小热修） | [x] |
