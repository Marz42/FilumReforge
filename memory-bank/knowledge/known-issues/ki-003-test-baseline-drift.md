...
| backend current main | **488 collected / 456 passed / 32 skipped / 0 failed** @ 2026-08-12 | Iteration 5-C shadow comparison、隐私/lag/孤儿/保留/worker 隔离、BIGINT 迁移与 SQLite expand/downgrade 后的完整汇总 |
...
| frontend current main | **73 files / 211 tests PASS** @ 2026-08-12 | F-05 工作流展示收口后 type-check、production build PASS |
...
**本地单元基线 ID**: `2026-08-12-iteration5c-shadow-comparison`；浏览器 E2E 最近完整基线为 `2026-08-09-v0.93.0-rc.1-release-candidate`，RC2/当前主线仍待目标环境刷新。

**Iteration 5-A/B/C 数据库门禁**：Alembic 单 head `20260812_03`；SQLite pre-projection expand/downgrade PASS；三段 PostgreSQL 增量离线 SQL（至 `20260812_03`）PASS。真实 PostgreSQL head↔base 因本机无 Docker/PostgreSQL 保持 skip，目标环境须用 `FILUM_REQUIRE_POSTGRES_TESTS=true` 补证；5-E 前还须运行 rebuild + full shadow 并保留持续样本。
