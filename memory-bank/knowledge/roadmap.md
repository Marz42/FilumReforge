...
| **阶段** | **主开发线 F-05 + RC2/UAT 等待** — 详情数据与动作协调拆分完成，I4/设计器/S-01 等待人工 UAT，I3-F 生产切流仍 gated |
| **最后整理** | 2026-08-11 — F-05 第二批动作协调完成，下一批资料/评论板块；KI-011 继续延后 |
...
| **F-05** | `TaskDetailShell` 完整拆分 | 🟡 数据与动作协调已抽取，约 1,989 → 1,569 行；资料附件与评论/留痕板块待后续 |
...
3. **F-05 后续拆分**：在保持界面与业务语义不变的前提下，下一批提取任务资料附件和评论/留痕板块；之后再评估活动时间线/工作流面板编排。
4. **员工 RC 复测**：升级至不可变 `v0.93.0-rc.2` 并核对 health version；用真实部门负责人验证共享模板可见/可发起且无越权管理动作。
5. **I3-F 生产门禁**：目标环境 Expand/Contract、Link 回填、恢复/回滚演练、连续 7 天观测与 31/31 报告。
6. **生产变更窗口**：完成 secret/TLS/备份恢复/迁移 dry-run/回滚预案后，按上线 checklist 批准部署。
7. **后续专项**：KI-011 按用户决定暂不推进；M-09 与 `run_kind` dual-read 收窄继续等待策略和生产证据。

**下一 actionable**：开发侧继续 F-05 资料附件与评论/留痕板块拆分；目标环境侧并行执行“数据检查”→“验收准备”→人工 UAT checklist。只有清单必测项完成并签字，才把 I4/设计器/S-01 改为 UAT 通过；RC 与生产准入仍按独立清单推进。
