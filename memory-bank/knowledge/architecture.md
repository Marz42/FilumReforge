...
- 工作流重构 Phase 2+：图引擎 schema 现为 **九表**（核心七表 + `workflow_run_events` + `workflow_graph_template_schedules`），含图模板状态、图实例状态、节点引擎态、节点业务投影态、outbox 事件状态枚举；as-built 总览见 [`domains/workflow-graph-engine.md`](./domains/workflow-graph-engine.md)。Phase 3 起已接入手动任务的 graph dual-write；任务中心待办 / 跟踪 / 历史在默认 `TASK_CENTER_V2_ENABLED=true` 下为 graph-first with legacy fallback（见 Phase 11-F）；B-12 已移除 Legacy E 对外 API、实例化入口与旧调度执行路径，`task_templates` 表族/ORM 暂保留用于历史数据兼容，不再是产品运行入口
